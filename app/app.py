...
def index():
    user = {'username': 'Final Project Part 1'}
...
def record_view(city_id):
    ...
    result = cursor.fetchall()
    return render_template('view.html', title='View Form', hws=result[0])
...
def form_edit_get(city_id):
    ...
    cursor.execute('SELECT * FROM bmiData WHERE id=%s', city_id)
    result = cursor.fetchall()
    return render_template('edit.html', title='Edit Form', hws=result[0])


@app.route('/edit/<int:city_id>', methods=['POST'])
def form_update_post(city_id):
    cursor = mysql.get_db().cursor()
    inputData = (request.form.get('Height_Inches'), request.form.get('Weight_Pounds'), city_id)
    sql_update_query = """UPDATE bmiData t SET t.Height_Inches = %s, t.Weight_Pounds = %s WHERE t.id = %s """
...
@app.route('/bmi/new', methods=['POST'])
...
def form_delete_post(city_id):
    cursor = mysql.get_db().cursor()
    sql_delete_query = """DELETE FROM bmiData WHERE id = %s """
...
@app.route('/api/v1/hws', methods=['POST'])
def api_add() -> str:
